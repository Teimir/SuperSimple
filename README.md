...
├── pipeline.py          # Общий фронтенд: препроцессор → лексер → парсер (build_ast)
├── version.py           # Версия проекта (__version__)
├── lexer.py             # Токенизатор (исходный код → токены)
├── parser.py            # Парсер (токены → AST)
├── interpreter.py      # Интерпретатор (AST → выполнение)
├── preprocessor.py     # Препроцессор (#include, #define, #undef)
...
- **main.py** — вызов pipeline.build_ast() и интерпретатор.
- **pipeline.py** — общая цепочка препроцессор → лексер → парсер; используется main.py и compile.py.
- **version.py** — единая версия для main.py и compile.py.
...
- **Рефакторинг:** общий фронтенд вынесен в `pipeline.py` (функция `build_ast`). Цепочка препроцессор → лексер → парсер используется и в `main.py`, и в `compile.py`, дублирование кода убрано.
- **Версия:** единый источник версии в `version.py`; `main.py` и `compile.py` выводят её по `-V` / `--version`.
- **Исправление:** корректная обработка отсутствующего файла — в пайплайне проверка `os.path.exists` и выброс `FileNotFoundError` с понятным сообщением.
- **README:** обновлены структура проекта, описание модулей, таймауты и коды выхода для `compile.py`, добавлен пример `--run` в начале аргументов.

### 1.0.0 и ранее

- **GUI удалён:** убраны `gui_main.py` и `emulator/gui.py`.
- Документация сведена в один README; обновлены устаревшие упоминания структуры.
- Примеры упрощены: объединены дубликаты (массивы, инкремент/декремент, циклы for), удалены избыточные папки.
- Массивы размещаются в памяти (data section); r30 — указатель программного стека для локальных переменных.
- Удалён отладочный код (debug.log); улучшены типизация и сообщения об ошибках в codegen, добавлены docstrings.
